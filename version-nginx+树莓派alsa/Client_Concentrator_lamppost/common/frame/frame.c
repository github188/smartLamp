...
{
    int offset = 0;
    unsigned char ucBeginChar = 0x68;//起始字符
    unsigned char ucEndChar = 0x16;//结束字符
    unsigned short usL = 0;//长度L
    char acAddr[5] = { 0 };//地址域A
    char A1[5] = { 0 };
    char A2[5] = { 0 };
    char A3 = 0;
	char szLogInfo[1024]  = { 0 };
    ...
    snprintf(szLogInfo, sizeof(szLogInfo) - 1, "The below information is parse_3761frame function[152 line]");
    ...
    A3 = acAddr[4];
    snprintf(frame->addr, sizeof(frame->addr), "%s%s%d", A1, A2, (char)(A3 >> 1));    // D1～D7组成0～127个主站地址
...
// 封装3761帧                                                                           //data: 为输出是真正的帧(从起始字符（68H）到结束字符（16H）)
int pack_3761frame(t_3761frame *frame, char *data, int len)//frame: 只包括链路用户数据 ，其他数据为空
{
    int offset = 0;
    unsigned char ucBeginChar = 0x68;//起始字符
    unsigned char ucEndChar = 0x16;//结束字符
    ...
    printf("Enter to pack_3761frame and over 1...\n");
    //printf("data = %s\n",data);
    // 起始字符（68H）
    ...
    usL = 12 + frame->user_data.len;  // 控制域、地址域、链路用户数据（应用层）的字节总数
                                     //12=起始字符(1)+长度L (2)+长度L (2)+起始字符(1)+
                                    //控制域C (1)+ 地址域A (5) (查看集中器上行协议V1.0)
    ...
    snprintf(szLogInfo, sizeof(szLogInfo) - 1, "The below information is pack_3761frame function[316 line]");
    WRITELOG(LOG_INFO, szLogInfo);
	//dumpInfo(data, offset);
